...
class BCEDiceLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        dice = 1.0 - ((2.0 * intersection + 1.0) /
                      (probs.sum(dim=dims) + target.sum(dim=dims) + 1.0)).mean()
...
class SemanticCrossEntropyDiceLoss(nn.Module):
    def __init__(self, num_classes: int, dice_weight: float = 1.0, ce_weight: float = 1.0):
        ...
        self.cross_entropy = nn.CrossEntropyLoss(ignore_index=ADE_IGNORE_INDEX)
...
def main():
    ...
    criterion = (
        SemanticCrossEntropyDiceLoss(num_classes=num_classes)
        if is_multiclass
        else BCEDiceLoss(args.dice_weight, args.bce_weight)
    )
